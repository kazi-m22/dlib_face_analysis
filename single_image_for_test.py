...
if len(dets) > 0:
    ...
    right_angle = round(math.degrees(math.atan(abs(right_lip[1] - left_nose[1]) / abs(left_nose[0] - right_lip[0]))), 3)
    left_angle = round(math.degrees(math.atan(abs(left_lip[1] - right_nose[1]) / abs(right_nose[0] - left_lip[0]))), 3)
    right_right_angle = round(math.degrees(math.atan(abs(right_lip[1] - right_nose[1]) / abs(right_nose[0] - right_lip[0]))),3)
    left_left_angle = round(math.degrees(math.atan(abs(left_lip[1] - left_nose[1]) / abs(left_nose[0] - left_lip[0]))),3)
    lef_l_e_angle = round(math.degrees(math.atan(abs(left_lip[1] - left_eye[1]) / abs(left_lip[0] - left_eye[0]))), 3)
    right_l_e_angle = round(math.degrees(math.atan(abs(right_lip[1] - right_eye[1]) / abs(right_lip[0] - right_eye[0]))), 3)

    write_list = [right_angle, right_right_angle, left_angle, left_left_angle, lip_ratio, eye_ratio]

    print(right_angle, right_right_angle, left_angle, left_left_angle, lip_ratio, eye_ratio, nose_ratio, lef_l_e_angle,right_l_e_angle)
